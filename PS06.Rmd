...
MSE <- mean((f(x0) - data1)^2)


variance <- var(data1)


bias.sq <- ((f(x0) - mean(data1$X1))^2)
...
```{r}
newdata <- data.frame(MSE= MSE, bias_squared= bias.sq, var= variance, irreducible= MSE - (bias.sq + variance), sum = bias.sq + variance)
...
1. For predicting one point of interest, I would do the first method, where $$\mbox{MSE}\left[\widehat{f}(x_0)\right]
= \mbox{MSE}\left[\widehat{f}(0.95)\right]$$ because the value will likely have less irreducible error because it did not account for the values of other points.
